...
public class EntryPoint {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ApiException, ServiceDownException {
		...
		User user = new User("admin", "123456", "Bob", "Smith", Gender.MALE, new Address.Builder().setStreet("1 Heaven Ave").setPostalCode("007").build(), new UserRole("admin", RoleRule.ADMIN, false, new UserPermission("admin.read.test"), new UserPermission("user.op"), new UserPermission("user.read.lm"), new UserPermission("user.test")).addAlias("user"));
